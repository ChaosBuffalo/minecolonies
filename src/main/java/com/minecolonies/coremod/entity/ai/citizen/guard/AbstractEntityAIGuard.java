    ...
    protected EntityLivingBase getTarget()
    {
        ...
        if (building != null && target == null)
        {
            ...
            for (final EntityLivingBase entity : targets)
            {
                if (entity instanceof EntityPlayer)
                {
                    ...
                    if (worker.getCitizenColonyHandler().getColony() != null && worker.getCitizenColonyHandler().getColony().getPermissions().hasPermission(player, Action.GUARDS_ATTACK)
                            && Targeting.isValidTarget(Targeting.TargetType.ENEMY, this.worker, entity, true)
                            && this.worker.getEntitySenses().canSee(player))
                    {
                        return entity;
                    }
            ...
            for (final EntityLivingBase entity : targets){
                ...
                boolean inBuildingMobs = false;
                for (final MobEntryView mobEntry : building.getMobsToAttack()) {
                    if (mobEntry.hasAttack() && mobEntry.getEntityEntry().getEntityClass().isInstance(entity)
                            && ( worker.getDistance(entity) < closest || (int) closest == -1)
                            && worker.getEntitySenses().canSee(entity)){
                ...
                if (!inBuildingMobs){
                    if ( entity.isEntityAlive() && Targeting.isValidTarget(Targeting.TargetType.ENEMY, this.worker, entity, true)
                            && ( worker.getDistance(entity) < closest || (int) closest == -1)
                            && worker.getEntitySenses().canSee(entity)){
                        closest = worker.getDistance(entity);
                        targetEntity = entity;
            ...
            if (targetEntity != null && Targeting.isValidTarget(Targeting.TargetType.ENEMY,
                    this.worker, targetEntity, true))
            {
                return targetEntity;
            }
        }
        ...
        {
            if (Targeting.isValidTarget(Targeting.TargetType.ENEMY, this.worker, target, true)){
                return target;
            }
        }
